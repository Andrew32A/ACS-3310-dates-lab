...
const bday = new Date("June 2, 1998");
...
const days2 = hrs / 24; // get days
const yrs = days2 / 365.25; // get years
...
function consecutiveDates(date, repeat, offset) {
  ...
  for (let i = 1; i < repeat; i++) {
    ...
    newDate.setDate(newDate.getDate() + i * offset);
    dates.push(newDate);
  }
...
function nextDate(dates) {
  ...
  for (const date of dates) {
    if (date > today && (!closestFutureDate || date < closestFutureDate)) {
